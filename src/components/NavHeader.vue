        ...
        methods:{
            checkLogin() { //检查是否登录
                axios.post('users/checkLogin').then(response=>{
                    ...
                    if(res.status == "0") {  //已经登录的页面
                        this.nickName = res.result.nickName || res.result.userName;
                        this.$store.commit('updateCartCount', !res.result.cartList.length?"":res.result.cartList.length);
                        //this.cartCount = res.result.cartList.length;
                        this.loginModalFlag = false;
                    }else if(res.status == "2") { //必须要登录的页面
                        window.alert("请先登录");
