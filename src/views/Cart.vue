...
<script>
    ...
    export default {
        ...
        computed: {
            checkAllFlag() {
              console.log(this.checkedCount+" "+this.cartList.length);
        ...
        methods:{
            init() {
                axios.get('/users/cartList').then((response)=>{
                    ...
                    if(res.status == "0") {
                        ...
                        this.$store.commit('updateCartCount', !res.result.length?"":res.result.length);
                    }else {
                        alert(res.msg);
                        location.href = "/";
                    }
            ...
            delCart(item) {
                axios.post('users/cart/del',{productId:this.delItem.productId}).then((response)=>{
                    let res = response.data;
                    if(res.status == "0") {
