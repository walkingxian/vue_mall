        ...
        <div class="container">
          <div class="checkout-addr">
            ...
            <div class="addr-list-wrap">
              <div class="addr-list">
                <ul>
                  <li v-for="(item,index) in addressListFilter" v-bind:class="{'check':checkIndex==index}" @click="setChecked(index,item)">
                    <dl>
                      <dt>{{item.userName}}</dt>
            ...
            init() {
                axios.get('users/addressList').then(response=>{
                    ...
                    if(res.status == "0") {
                        ...
                        this.selectedAddrId = res.result[0].addressId;
                    }else {
                        alert(res.msg);
                    }
                })
        ...
        computed:{
            addressListFilter() {
                ...
                this.addressList.forEach((item)=>{
                  if(item.isDefault) {
                    filterAddrLists.unshift(item);
                  }else {
                    filterAddrLists.push(item);
                  }
                })
                return filterAddrLists.slice(0,this.limit);
            }
        }
    }
</script>
