        ...
        <div class="item-list-wrap confirm-item-list-wrap">
          <div class="cart-item order-item">
            ...
            <ul class="cart-item-list">
              <li v-for="item in checkedCartList">
        ...
        methods:{
            init() {
                axios.get('/users/cartList').then(response=>{
                    ...
                    if(res.status == "0") {
                        this.checkedCartList = res.result.filter(item=>{
                            this.shipping += item.shipping * item.productNum;
                            this.tax += item.tax * item.productNum;
                            this.discount += item.discount * item.productNum;
                            this.subTotal += item.salePrice * item.productNum;
                            this.orderTotal += (parseFloat(item.salePrice) + item.shipping + item.tax - item.discount) * item.productNum;
